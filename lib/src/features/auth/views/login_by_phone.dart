...
class LoginByPhone extends StatefulWidget {
  ...
  const LoginByPhone({super.key, required this.isCreateAccount, this.sessionId});
      ...
      child: Column(
        children: [
          ...
          DesignSystem.avatarTitleSubtitle(
            title: isCreateAccount ? SMText.registerNow : SMText.loginNow2,
                ...
                BlocConsumer<AuthCubit, AuthState>(
                  ...
                  listener: (context, state) {
                    if (state.sendOtpStatus.isFailure) {
                      primarySnackBar(context, message: state.errorMessage ?? 'Failed to send OTP. Please try again.');
                    }
                  ...
                  builder: (context, state) => ValueListenableBuilder(
                    ...
                    builder: (context, value, child) => DesignSystem.primaryButton(
                      borderRadius: 14,
                      title: isCreateAccount ? SMText.registerNow : SMText.login,
                      ...
                      onPressed: () async {
                        ...
                        sl<AuthCubit>().sendOtp(countryCode: country.dialCode, phone: phoneNumber, sessionId: widget.sessionId);
                      },
                    ),
                  ),
                ),
